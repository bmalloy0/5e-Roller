...
public class GenerateNPCSimple : MonoBehaviour {
    ...
    public void Create(ref NPCBlock blocker)
    {
        if (monsterNPC.isOn)
        {
            blocker.race = npcRace.RaceMonster();
        }
        else
        {
            blocker.race = npcRace.Race(ref block);
        }

        blocker.npcName = namer.Input(ref blocker);
    ...
    void Start()
    {
        ...
        npcJob = GetComponent<NPCJob>();
        block = new NPCBlock();
        namer = GetComponent<NPCName>();
        npcRace = GetComponent<NPCRace>();
    }
}
